...
import 'package:taxi_driver/app/model/FRideBookingModel.dart';
...
class RideService extends BaseService {
  ...
  Stream<QuerySnapshot> fetchRide({int? userId}) {
    ...
    // return rideRef.where('drivers_id', arrayContains: userId).snapshots();
    var driverIdQuery = rideRef
        .where(
          Filter.or(
            Filter('driver_id', isEqualTo: userId),
            Filter('drivers_id', arrayContains: userId),
          ),
        )
        .snapshots();

    return driverIdQuery;
  }

  Future<bool> removeOldRideEntry({int? userId}) async {
    try {
      QuerySnapshot<Object?> b =
          await rideRef.where('driver_id', isEqualTo: userId).get();
      List<FRideBookingModel> x = b.docs
          .map((e) =>
