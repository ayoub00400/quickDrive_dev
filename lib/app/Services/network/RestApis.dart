...
import 'package:taxi_driver/app/model/earning_model.dart';
import 'package:taxi_driver/app/model/state_model.dart';
import 'package:taxi_driver/app/utils/Extensions/StringExtensions.dart';

import '../bg_notification_service.dart';
import '../../../main.dart';
...
import '../../view/screens/SignInScreen.dart';
...
Future<List<StateModel>> getStateList() async {
  return (await handleResponse(await buildHttpResponse('getStates', method: HttpMethod.GET)))['states']
...
Future<void> logout({bool isDelete = false}) async {
  if (!isDelete) {
    await logoutApi().then((value) async {
      logOutSuccess();
      await NotificationWithSoundService.flutterLocalNotificationsPlugin.cancelAll();
...
Future<DocumentListModel> getDocumentList() async {
  return DocumentListModel.fromJson(
      await handleResponse(await buildHttpResponse('document-list', method: HttpMethod.GET)));
}
...
Future<AdditionalFeesList> getAdditionalFees() async {
  return AdditionalFeesList.fromJson(
      await handleResponse(await buildHttpResponse('additional-fees-list?status=1', method: HttpMethod.GET)));
}
...
Future<RideDetailModel> rideDetail({required int? rideId}) async {
  return RideDetailModel.fromJson(
      await handleResponse(await buildHttpResponse('riderequest-detail?id=$rideId', method: HttpMethod.GET)));
}
...
Future<void> sendTripPriceToRider({required Map request}) async {
  var value = await buildHttpResponse('driver-offer', method: HttpMethod.POST, request: request);
...
Future<EarningModel> totalEarning({Map? req}) async {
  return EarningModel.fromJson(
      await handleResponse(await buildHttpResponse('totalEarning', method: HttpMethod.GET, request: req)));
}
...
Future<WalletDetailModel> walletDetailApi() async {
  return WalletDetailModel.fromJson(
      await handleResponse(await buildHttpResponse('wallet-detail', method: HttpMethod.GET)));
}

Future<LDBaseResponse> complaintComment({required Map request}) async {
  return LDBaseResponse.fromJson(await handleResponse(
      await buildHttpResponse('save-complaintcomment', method: HttpMethod.POST, request: request)));
}

Future<ComplaintCommentModel> complaintList({required int complaintId, required int currentPage}) async {
  return ComplaintCommentModel.fromJson(await handleResponse(await buildHttpResponse(
