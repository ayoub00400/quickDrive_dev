...
import 'package:taxi_driver/app/utils/Extensions/Loader.dart';
...
InputDecoration inputDecoration(BuildContext context,
    {String? label, Widget? prefixIcon, Widget? suffixIcon, String? counterText}) {
  return InputDecoration(
    ...
    border: OutlineInputBorder(
        borderRadius: BorderRadius.circular(defaultRadius), borderSide: BorderSide(color: dividerColor)),
    focusedErrorBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(defaultRadius), borderSide: BorderSide(color: dividerColor)),
    disabledBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(defaultRadius), borderSide: BorderSide(color: dividerColor)),
    focusedBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(defaultRadius), borderSide: BorderSide(color: Colors.black)),
    enabledBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(defaultRadius), borderSide: BorderSide(color: dividerColor)),
    errorBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(defaultRadius), borderSide: BorderSide(color: Colors.red)),
  ...
  if (data != null && data.uid != null) {
    return Stack(
      children: [
        ...
        StreamBuilder<int>(
            stream:
                chatMessageService.getUnReadCount(receiverId: "${data.uid}", senderId: "${sharedPref.getString(UID)}"),
...
oneSignalSettings() async {
  ...
  const String notificationTitle = 'Hisba Makhzoun';
  const String notificationSound = 'notification_sound';

  const oneNotificationChannel = AndroidNotificationChannel(
    channelId,
    channelName,
  ...
  const oneNotificationChannel1 = AndroidNotificationChannel(
    '${channelId}1',
    '${channelName}1',
  ...
  const groupNotificationChannel = AndroidNotificationChannel(
    '${channelId}2',
    '${channelName}2',
  ...
  try {
    OneSignal.User.pushSubscription.addObserver((state) async {
      ...
      if (OneSignal.User.pushSubscription.id.validate().isNotEmpty)
        await sharedPref.setString(PLAYER_ID, OneSignal.User.pushSubscription.id.validate());
    });
  } catch (e, s) {
    print("TEST::::776");
    print("TEST:::ERR:::$e ==>$s");
